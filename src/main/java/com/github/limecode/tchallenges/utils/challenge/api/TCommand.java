...
public abstract class TCommand<T extends JavaPlugin> extends Command implements CommandExecutor, PluginIdentifiableCommand {
    ...
    protected TCommand(JavaPlugin plugin, String name) {
        super(name);
        ...
        setLabel(name);
        setPermission(String.format("tchallenges.%s", name));
        this.plugin = plugin;
        tabComplete = new HashMap<>();
        permissionMessage(Component.text("§cDu hast nicht genug Berechtigungen, um diesen Command zu benutzen!"));
    }
